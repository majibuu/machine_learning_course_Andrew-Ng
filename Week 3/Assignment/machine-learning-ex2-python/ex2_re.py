...
from gradient_reg import compute_gradient_reg
...
initial_theta = np.zeros((X.shape[1]),dtype=float)
cost, grad = cost_function_reg(initial_theta,X,y,lamda,True)
print('Cost at initial theta (zeros): {}'.format(cost))
print('Gradient at initial theta (zeros): {}'.format(grad))
myargs=(X, y, lamda,True)
theta = sp.optimize.fmin_tnc(cost_function_reg, x0=initial_theta, args=myargs)
#myargs=(X, y, lamda)
#theta = sp.optimize.fmin_ncg(cost_function_reg, x0=initial_theta, fprime=compute_gradient_reg,args=myargs)
print(theta)
